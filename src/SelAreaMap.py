...
class SelAreaMap:
    ...
    def onclick(self, event):
        ...
        if self.click_count == 2:
            lower_left_corner = self.coords[0]
            upper_right_corner = self.coords[1]
            width = upper_right_corner[0] - lower_left_corner[0]
            height = upper_right_corner[1] - lower_left_corner[1]
            rect = Rectangle(lower_left_corner, width, height, edgecolor='red', facecolor='none', linewidth=2, transform=ccrs.PlateCarree())
            self.ax.add_patch(rect)
            self.fig.canvas.draw()

            file = f'sel_area2_{self.region}.png'
...
regiao = input("Escolha uma região: ['nordeste','sudeste','sul'] \nMinha região é: ")

# Executar função
mapa = SelAreaMap(dataset_path, regiao)
